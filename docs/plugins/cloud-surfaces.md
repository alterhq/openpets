...
The first-party battery plugin is the canonical built-in example. It stays in the OpenPets repository until the external plugin runtime and install contract are stable.
...
Multi-metric plugins should emit one surface per metric. For example, Claude usage can emit separate `claude.5h` and `claude.7d` cloud surfaces, with reset times and pace in click details.
...
- Keep the battery plugin enabled as a first-party built-in plugin.
...
- Build a Claude Status plugin next to validate multiple metric clouds and click details.
